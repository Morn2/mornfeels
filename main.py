...
from utils import save_widget_as_image, generate_pdf_from_images
...
class ReminderPopup(Popup):
    ...
    def __init__(self, file_path, **kwargs):
        ...
        layout = FloatLayout()
        label = Label(text="How are you feeling?",
        ...
        note_label = Label(text="Additional Note (optional)",
...
class VisualizationResultsPopup(Popup):
    """
    Popup, das das LineChartWidget anzeigt und optional als PDF speichert.
    """
    def __init__(self, filtered_data, start_str, end_str, **kwargs):
        super().__init__(**kwargs)
        ...
        # --> Übergib hier die Spinner-Werte an LineChartWidget!
        self.chart_widget = LineChartWidget(filtered_data, start_str, end_str)
...
class VisualizePopup(Popup):
    ...
    def on_generate(self, instance):
        ...
        if self.checkbox_line.active:
            # Erzeuge Popup für das Liniendiagramm, übergebe Start- und Enddatum!
            results_popup = VisualizationResultsPopup(filtered_data, start_date, end_date)
            results_popup.open()
        ...
        # Wenn wir Pie Charts / Bar Charts erzeugt haben, wollen wir sie ggf. separat anzeigen
        if image_paths:
            # Du könntest hier ein anderes Popup machen, das die Bilder anzeigt.
            # Momentan wird in deinem Code dasselbe Popup aufgerufen.
            # Zum Beispiel so (pseudo-code):
            #
            # image_popup = VisualizationResultsPopupForImages(image_paths)
            # image_popup.open()
            #
            # Da dieses Popup bei dir aber noch nicht existiert,
            # lassen wir es erstmal weg oder machen gar nichts.
            pass
